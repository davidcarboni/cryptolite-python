...
def to_hex_string(byte_array):
    """Renders the given byte array as a hex String.
     ...
     :param byte_array: The byte array to be rendered.
...
def from_hex_string(hex_string):
    ...
    :param hex_string: The hex String to parse. If it starts with 0x, this will be
                 ignored automatically.
    :return: A byte array, as parsed from the given String
    """
    result = None
    if hex_string is not None:
        if len(hex_string) > 1 and hex_string[1] == "x":
...
def from_string(string):
    ...
    if string is not None:
        result = bytearray(string.encode(ENCODING))
    return result
